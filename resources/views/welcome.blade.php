<!DOCTYPE html>
<html lang="en">

<head>
  <title>Sama Dentiste</title>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

  <link rel="stylesheet" href="{{asset('accueil/fonts/icomoon/style.css')}}">

  <link rel="stylesheet" href="{{asset('accueil/css/bootstrap.min.css')}}">
  <link rel="stylesheet" href="{{asset('accueil/fonts/flaticon/font/flaticon.css')}}">
  <link rel="stylesheet" href="{{asset('accueil/css/magnific-popup.css')}}">
  <link rel="stylesheet" href="{{asset('accueil/css/jquery-ui.css')}}">
  <link rel="stylesheet" href="{{asset('accueil/css/owl.carousel.min.css')}}">
  <link rel="stylesheet" href="{{asset('accueil/css/owl.theme.default.min.css')}}">


  <link rel="stylesheet" href="{{asset('accueil/css/aos.css')}}">

  <link rel="stylesheet" href="{{asset('accueil/css/style.css')}}">

</head>

<body>

  <div class="site-wrap">


    <div class="site-navbar py-2">

      <div class="search-wrap">
        <div class="container">
          <a href="#" class="search-close js-search-close"><span class="icon-close2"></span></a>
          <form action="#" method="post">
            <input type="text" class="form-control" placeholder="Search keyword and hit enter...">
          </form>
        </div>
      </div>

      <div class="container">
        <div class="d-flex align-items-center justify-content-between">
          <div class="logo">
            <div class="site-logo">
              <a  class="js-logo-clone"><strong class="text-primary">Sama</strong>Dentiste</a>
            </div>
          </div>

          
          <div class="main-nav d-none d-lg-block">
            <nav class="site-navigation " >
              <ul class="site-menu js-clone-nav d-none d-lg-block">
                <li>
                    <a href="#" class="icons-btn d-inline-block js-search-open"><span class="icon-search"></span></a>
                    </li>
                <li class="has-children">
                    <a href="#"  ><span class="icon-user-md"></span>
                    </a>
                    <ul class="dropdown" >
                        @if (Route::has('login'))
                                    @auth
                                        <li><a href="{{ url('/dashboard') }}" >Dashboard</a></li>
                                        
                                        <li>
                                          <a href="{{ url('logout') }}" 
                                          onclick="event.preventDefault();
                                          document.getElementById('logout-form').submit();">
                                          {{ __('Log Out') }}
                                          </a>
                                          
                                          <form id="logout-form" method="POST" action="{{ route('logout') }}" style="display: none;">@csrf</form>
                                          
                                        </li>
                                    @else
                                        <li><a href="{{ route('login') }}" >Log in</a></li>

                                    @endauth
                        @endif
                        
                    </ul>
                    <a href="#" class="site-menu-toggle js-menu-toggle ml-3 d-inline-block d-lg-none"><span
                        class="icon-menu"></span></a>

                </li>
              </ul>
            </nav>
          </div>
        </div>
      </div>
    </div>

    <div class="site-blocks-cover overlay" style="background-image: url({{url('accueil/images/hero_bg_2.jpg')}});">
      <div class="container">
        <div class="row">
          <div class="col-lg-12 mx-auto align-self-center">
            <div class="site-block-cover-content text-center">
              <h1 class="mb-0"><strong class="text-primary">Sama</strong>Dentiste</h1>
              <div class="row justify-content-center mb-5">
                <div class="col-lg-6 text-center">
                  <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facilis ex perspiciatis non quibusdam vel quidem.</p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    
    
    <div class="site-section py-5" data-aos="fade">
      <div class="container">
        <div class="row">
          <div class="col-lg-4">
            <h3 class="text-black h4">Why Us</h3>
            <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Rem assumenda, delectus. Amet repellendus quidem, fugiat.</p>
            
          </div>
          <div class="col-lg-4">
            <h3 class="text-black h4">History</h3>
            <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Assumenda iste aut, ut similique nobis ab?</p>
            
          </div>
          <div class="col-lg-4">
            <h3 class="text-black h4">Testimony</h3>
            <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Repudiandae nisi magni in fugit, ad laudantium.</p>
            
          </div>
        </div>
      </div>
    </div>    

    <div class="site-section bg-light custom-border-bottom" data-aos="fade">
      <div class="container">
        <div class="row justify-content-center mb-5">


          <div class="title-section text-center col-md-7">
            <h2>Our <strong class="text-primary">Dentist</strong></h2>
          </div>

        </div>
        <div class="row">
          <div class="col-md-6 col-lg-4 mb-5">
    
            <div class="block-38 text-center">
              <div class="block-38-img">
                <div class="block-38-header">
                  <img src="{{asset('accueil/images/person_1.jpg')}}" alt="Image placeholder" class="mb-4">
                  <h3 class="block-38-heading h4">Dr Chopper</h3>
                  <p class="block-38-subheading">dentist</p>
                </div>
                <div class="block-38-body">
                  <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Vitae aut minima nihil sit distinctio
                    recusandae doloribus ut fugit officia voluptate soluta. </p>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-6 col-lg-4 mb-5">
            <div class="block-38 text-center">
              <div class="block-38-img">
                <div class="block-38-header">
                  <img src="{{asset('accueil/images/person_2.jpg')}}" alt="Image placeholder" class="mb-4">
                  <h3 class="block-38-heading h4">Dr Tsunade</h3>
                  <p class="block-38-subheading">dentist</p>
                </div>
                <div class="block-38-body">
                  <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Vitae aut minima nihil sit distinctio
                    recusandae doloribus ut fugit officia voluptate soluta. </p>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-6 col-lg-4 mb-5">
            <div class="block-38 text-center">
              <div class="block-38-img">
                <div class="block-38-header">
                  <img src="{{asset('accueil/images/person_3.jpg')}}" alt="Image placeholder" class="mb-4">
                  <h3 class="block-38-heading h4">Dr Kureha</h3>
                  <p class="block-38-subheading">dentist</p>
                </div>
                <div class="block-38-body">
                  <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Vitae aut minima nihil sit distinctio
                    recusandae doloribus ut fugit officia voluptate soluta. </p>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-6 col-lg-4 mb-5">
            <div class="block-38 text-center">
              <div class="block-38-img">
                <div class="block-38-header">
                  <img src="{{asset('accueil/images/person_4.jpg')}}" alt="Image placeholder" class="mb-4">
                  <h3 class="block-38-heading h4">Dr Shizune</h3>
                  <p class="block-38-subheading">dentist</p>
                </div>
                <div class="block-38-body">
                  <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Vitae aut minima nihil sit distinctio
                    recusandae doloribus ut fugit officia voluptate soluta. </p>
                </div>
              </div>
            </div>
          </div>

          <div class="col-md-6 col-lg-4 mb-5">
    
            <div class="block-38 text-center">
              <div class="block-38-img">
                <div class="block-38-header">
                  <img src="{{asset('accueil/images/person_6.jpg')}}" alt="Image placeholder" class="mb-4">
                  <h3 class="block-38-heading h4">Dr Hiluluk</h3>
                  <p class="block-38-subheading">dentist</p>
                </div>
                <div class="block-38-body">
                  <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Vitae aut minima nihil sit distinctio
                    recusandae doloribus ut fugit officia voluptate soluta. </p>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-6 col-lg-4 mb-5">
            <div class="block-38 text-center">
              <div class="block-38-img">
                <div class="block-38-header">
                  <img src="{{asset('accueil/images/person_5.jpg')}}" alt="Image placeholder" class="mb-4">
                  <h3 class="block-38-heading h4">Dr Sakura</h3>
                  <p class="block-38-subheading">dentist</p>
                </div>
                <div class="block-38-body">
                  <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Vitae aut minima nihil sit distinctio
                    recusandae doloribus ut fugit officia voluptate soluta. </p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>


    <footer class="site-footer bg-light">
      <div class="container">
        <div class="row">
          <div class="col-md-6 col-lg-4 mb-4 mb-lg-0">

            <div class="block-7">
              <h3 class="footer-heading mb-4">Sama <strong class="text-primary">Clinique</strong></h3>
              <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Eius quae reiciendis distinctio voluptates
                sed dolorum excepturi iure eaque, aut unde.</p>
            </div>

          </div>
          <div class="col-lg-2 mx-auto mb-5 mb-lg-0">
          </div>

          <div class="col-md-6 col-lg-4">
            <div class="block-5 mb-5">
              <h3 class="footer-heading mb-4">Contact Info</h3>
              <ul class="list-unstyled">
                <li class="address">Harlem Keur Ndiaye Lô, Rufisque, Dakar, Sénégal</li>
                <li class="phone"><a href="tel://23923929210">+221 000 00 00</a></li>
                <li class="email">Gakoudev@gmail.com</li>
              </ul>
            </div>


          </div>
        </div>
        <div class="row pt-5 mt-5 text-center">
          <div class="col-md-12">
            <p>
              Copyright &copy;
              <script>document.write(new Date().getFullYear());</script> <i class="icon-heart text-danger" aria-hidden="true"> </i><a href="#" target="_blank"
                class="text-primary">Gakoudev</a>
              </p>
          </div>

        </div>
      </div>
    </footer>
  </div>

  <script src="{{asset('accueil/js/jquery-3.3.1.min.js')}}"></script>
  <script src="{{asset('accueil/js/jquery-ui.js')}}"></script>
  <script src="{{asset('accueil/js/popper.min.js')}}"></script>
  <script src="{{asset('accueil/js/bootstrap.min.js')}}"></script>
  <script src="{{asset('accueil/js/owl.carousel.min.js')}}"></script>
  <script src="{{asset('accueil/js/jquery.magnific-popup.min.js')}}"></script>
  <script src="{{asset('accueil/js/aos.js')}}"></script>

  <script src="{{asset('accueil/js/main.js')}}"></script>

</body>

</html>